pipeline {
    ...
    environment {
        ...
        withCredentials([usernamePassword(credentialsId: 'github', passwordVariable: 'GIT_PASSWORD', usernameVariable: 'GIT_USERNAME')]){
            stable_revision = sh(script: 'curl -u $GIT_USERNAME:$GIT_PASSWORD "https://api.enterprise.apigee.com/v1/organizations/${org}/apis/${proxy}/deployments" | jq -r ".environment[0].revision[0].name"', returnStdout: true).trim()
        }
    }
    stages {
